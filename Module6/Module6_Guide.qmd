...
<!-- PLACEHOLDER: Record lecture on LISA trajectories, space-time Moran's I, trend analysis, and policy design -->
...
In Lab 6 you will extend the temporal analysis from Lab 5 into a full spatio-temporal analysis. Using the same 2010 and 2019 datasets (same boundaries, same pooled standardization), you will:

1. **Run LISA on both periods**: Classify each CBG into HH/LL/HL/LH/NS for both 2010 and 2019
2. **Build the transition matrix**: Cross-tabulate cluster membership to identify trajectories
3. **Map trajectories**: Produce a 6-category trajectory map with policy-interpretable colors
4. **Space-time Moran's I**: Test whether change scores are spatially autocorrelated; interpret the result for spatial diffusion
5. **Three-point trend**: Pull the 2016 ACS (`year = 2016`, also on 2010 boundaries) and classify each CBG as consistently improving, worsening, or non-monotone
6. **Policy brief**: Write a structured paragraph recommending how to allocate a hypothetical intervention budget given the trajectory map

**Connection to Module 5:** Lab 5 showed *where* hardship changed. Lab 6 shows whether change is *spatially structured* — whether decline and recovery are spreading processes or idiosyncratic neighborhood events.
