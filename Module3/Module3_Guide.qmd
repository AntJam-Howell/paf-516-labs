...
After joining, group by the polygon ID and count matched points to produce a per-area count variable you can add to the hardship index.
...
In Lab 3, Q2 asks you to change the buffer from 1 mile to 0.5 miles and compare results. This is a direct application of UGCoP: you are testing whether the conclusion that "certain block groups are near facilities" is sensitive to the distance threshold. If the enriched hardship index changes substantially at 0.5 miles, UGCoP is at work.
...
Engage with at least two classmates' posts. Look for cases where a different buffer distance or data source would change the interpretation.
