...
In Modules 1--3, you built and mapped a hardship index. You probably noticed clusters --- groups of nearby block groups with similarly high or low values. But are those clusters **statistically significant**, or could they have occurred by chance? This module gives you the tools to answer that question.
...
<!-- PLACEHOLDER: Record lecture on spatial autocorrelation, Moran's I, and LISA cluster interpretation -->
...
In Lab 04, you will apply spatial statistics to your hardship index to identify where high- and low-hardship neighborhoods cluster. Specifically, you will:

1. **Build a spatial weights matrix**: Construct queen contiguity neighbors from your CBG boundaries and convert to a row-standardized weights list
2. **Test Global Moran's I**: Compute the global statistic and interpret whether your hardship index is significantly spatially clustered
3. **Compute Local Moran's I (LISA)**: Calculate local statistics for each block group and classify them into HH, LL, HL, LH, or not significant
4. **Create a LISA cluster map**: Map the four cluster types using the conventional color scheme (red = HH hot spots, blue = LL cold spots, pink = HL outliers, light blue = LH outliers, gray = not significant)
5. **Interpret the results**: Write a brief analysis identifying where hot spots and cold spots occur and what demographic or geographic factors might explain them

**Connection to previous labs:** This module statistically validates the spatial patterns you observed visually in Labs 1--3. The hot spots you identify here will be tested for temporal stability in Lab 05.

Download the [Lab 04 Quarto template (.qmd)](https://raw.githubusercontent.com/AntJam-Howell/paf-516-labs/main/Lab4/Assignment/Lab4_Assignment.qmd), complete the exercises, and submit both the .qmd and rendered .html to Canvas.

**Download:** [Lab4_Assignment.qmd](https://raw.githubusercontent.com/AntJam-Howell/paf-516-labs/main/Lab4/Assignment/Lab4_Assignment.qmd)
...
Consider the LISA cluster map you will produce in Lab 4, along with this module's readings on spatial dependence and Tobler's First Law. Then write a substantive post (minimum 150 words) that addresses the following:

**Prompt:** Hot spots (HH clusters) in your LISA map identify neighborhoods where high hardship is spatially concentrated --- surrounded by other high-hardship areas. Cold spots (LL clusters) identify the opposite. But clustering is a *pattern*, not an *explanation*. Drawing on what you know about your study area (Maricopa County), the readings on spatial autocorrelation, and any relevant policy or historical knowledge, discuss: What structural, institutional, or historical processes might produce the spatial clustering of hardship you observe? How should the distinction between hot spots, cold spots, and spatial outliers inform the *design* of policy interventions --- should resources be allocated differently to each type? Finally, reflect on what a spatial outlier (e.g., a high-hardship block group surrounded by low-hardship neighbors) might reveal about the limitations of place-based policy.
