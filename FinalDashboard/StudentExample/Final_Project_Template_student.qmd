...
library(viridis); library(tigris); library(DT)
library(mapgl); library(plotly); library(scales); library(MASS)
```

```{r}
#| label: county-config
# ── STUDENT: change these 3 lines only ──────────────────────────────────
# Update the YAML subtitle above to match your county.
# Examples:
...
#   Harris/Houst: TARGET_STATE="TX", TARGET_COUNTY="Harris",       TARGET_LABEL="Harris County, TX"
TARGET_STATE  <- "CA"
...
#| label: pull-national
#| cache: true
# National county data — cached because it does not depend on TARGET_STATE/COUNTY
county_raw <- get_acs(
  geography    = "county",
...
# All tracts in TARGET_STATE — NOT cached so TARGET_STATE changes propagate
...
# Target county boundary — derived from tract union for yellow outline
target_boundary <- az_tract_2023 %>%
...
```{r}
#| label: lisa-az
# State-wide LISA — NOT cached so TARGET_STATE changes propagate
...
# Target county tracts (2023) — NOT cached so TARGET_COUNTY changes propagate
...
renter_tbl <- local_combined_raw %>%
  ...
  st_drop_geometry() %>% dplyr::select(-moe) %>%
  pivot_wider(names_from = variable, values_from = estimate) %>%
  mutate(renter_rate = renter_num / renter_den) %>%
  dplyr::select(GEOID, renter_rate)
local_2023 <- left_join(local_2023, renter_tbl, by = "GEOID")
```

```{r}
#| label: pull-temporal
# Temporal pulls for trajectory analysis — NOT cached
...
r2013 <- local_2013_wide %>% st_drop_geometry() %>%
  dplyr::select(GEOID, poverty_rate, unemp_rate, median_income)
...
change_df <- local_2019_wide %>%
  ...
  left_join(r2019_z %>% dplyr::select(GEOID, hardship_2019 = hardship_index), by = "GEOID") %>%
  inner_join(r2013_z %>% dplyr::select(GEOID, hardship_2013 = hardship_index), by = "GEOID") %>%
...
r2016_z <- apply_pooled_z(
  local_2016_wide %>% dplyr::select(GEOID, poverty_rate, unemp_rate, median_income),
  pooled_stats
)
...
change_df <- change_df %>%
  ...
  left_join(r2016_z %>% dplyr::select(GEOID, hardship_2016 = hardship_index), by = "GEOID")
```
...
# 2013->2023 change using local_2023
maricopa_change_23 <- change_df %>%
  st_drop_geometry() %>%
  dplyr::select(GEOID, hardship_2013, trajectory) %>%
  left_join(
    local_2023 %>% st_drop_geometry() %>% dplyr::select(GEOID, hardship_2023 = hardship_index),
    by = "GEOID"
  ) %>%
...
us_ranks <- county_2023 %>%
  ...
  dplyr::select(GEOID, us_rank)

# State-level county table
state_full <- state.name[match(TARGET_STATE, state.abb)]
state_counties <- county_2023 %>%
  st_drop_geometry() %>%
  filter(str_detect(NAME, state_full)) %>%
  left_join(us_ranks, by = "GEOID") %>%
  arrange(desc(hardship_index)) %>%
  mutate(
    state_rank  = row_number(),
    n_state     = n(),
    county_name = str_remove(NAME, paste0(" County, ", state_full)),
    County = case_when(
      county_name == TARGET_COUNTY ~ paste0(county_name, " \u2190 your county"),
      state_rank == 1              ~ paste0(county_name, " (Worst in state)"),
      state_rank == n_state        ~ paste0(county_name, " (Best in state)"),
      TRUE                         ~ county_name
    ),
    `Economic Hardship Index` = round(hardship_index, 3)
  ) %>%
  dplyr::select(`State Rank` = state_rank, County, `Economic Hardship Index`,
         `US Rank` = us_rank)

# Within-county heterogeneity table
...
bot5_us <- county_2023 %>%
  ...
  mutate(
    ...
    state_abb    = state.abb[match(state_name, state.name)],
    county_short = str_remove(NAME, " County.*"),
    group = "Bottom 5 US (Least Hardship)",
    label = paste0(county_short, ", ", coalesce(state_abb, state_name))
  )

state_all <- county_2023 %>%
  st_drop_geometry() %>%
  left_join(us_ranks, by = "GEOID") %>%
  filter(str_detect(NAME, state_full), !is.na(hardship_index)) %>%
  mutate(
    group = paste0(TARGET_STATE, " Counties"),
    label = paste0(str_remove(NAME, paste0(" County, ", state_full)), ", ", TARGET_STATE)
  )

compare_df <- bind_rows(top5_us, state_all, bot5_us) %>%
  mutate(
    color_group = case_when(
      group == paste0(TARGET_STATE, " Counties") ~ "#fdbf11",
      group == "Top 5 US (Most Hardship)"        ~ "#db2b27",
      TRUE                                        ~ "#1696d2"
    ),
    label = fct_reorder(label, hardship_index)
  )

state_mean <- mean(compare_df$hardship_index[
  compare_df$group == paste0(TARGET_STATE, " Counties")], na.rm = TRUE)
```
...
```{r}
#| label: map-national
#| title: "Economic Hardship Index: U.S. Counties (2023)"
...
maplibre(
  ...
  add_line_layer(
    id = "county-borders", source = county_map,
    line_color = "#333333", line_width = 0.2
  ) |>
  add_line_layer(
    id = "state-highlight", source = state_outline,
...
```{r}
#| label: table-state-counties
#| title: "State Counties — Economic Hardship Rankings"
state_counties %>%
  DT::datatable(
...
```{r}
#| label: chart-dot-compare
#| title: "State Counties vs. U.S. Extremes"
p <- ggplot(compare_df,
...
ggplotly(p, tooltip = "text") %>%
  layout(
    title  = list(
      text = paste0(state_full, " Counties vs. U.S. Extremes",
                    "<br><sup><span style=\"color:#fdbf11\">&#9679;</span> ", TARGET_STATE, " Counties &nbsp;",
                    "<span style=\"color:#db2b27\">&#9679;</span> 5 Highest US &nbsp;",
                    "<span style=\"color:#1696d2\">&#9679;</span> 5 Lowest US &nbsp;",
                    "<span style=\"color:#9d9d9d\">&#9474;</span> State Mean</sup>"),
    ...
    shapes = list(list(
      type = "line",
      x0 = state_mean, x1 = state_mean, y0 = 0, y1 = 1, yref = "paper",
...
# State in Focus {orientation="rows"}
...
```{r}
#| label: map-state-tracts
#| title: "Economic Hardship Index: State Census Tracts (2023)"
az_map <- az_tract_2023 %>%
  filter(!is.na(hardship_index)) %>%
  st_simplify(preserveTopology = TRUE, dTolerance = 500) %>%
  st_transform(4326)

# Compute AZ bounds dynamically from the data
az_bbox <- st_bbox(az_map)
map_bounds <- c(az_bbox$xmin - 0.3, az_bbox$ymin - 0.3,
                az_bbox$xmax + 0.3, az_bbox$ymax + 0.3)
...
maplibre(
  ...
  add_line_layer(
    id = "target-county-border", source = target_boundary,
    line_color = "#fdbf11", line_width = 2.5
  ) |>
  htmlwidgets::onRender("
    function(el, x) {
      ...
      badge.innerHTML = '&#9632;&nbsp;Target county boundary';
...
#| title: "Poverty vs. Unemployment: State Census Tracts"
...
scatter_df <- az_tract_2023 %>%
  ...
  mutate(
    is_target  = county_name == TARGET_COUNTY,
    tract_label = paste0(
      str_extract(NAME, "Census Tract [\\d\\.]+"), "\n",
...
get_dens <- function(x, y, d) {
  xi <- pmax(1L, pmin(findInterval(x, d$x), length(d$x) - 1L))
  yi <- pmax(1L, pmin(findInterval(y, d$y), length(d$y) - 1L))
  d$z[cbind(xi, yi)]
}
scatter_df <- scatter_df %>%
  mutate(pt_alpha = scales::rescale(get_dens(poverty_rate, unemp_rate, dens), to = c(0.55, 0.92)))

p_sc <- ggplot(scatter_df,
    aes(x = poverty_rate, y = unemp_rate, color = hardship_index,
        text = tract_label, size = is_target, alpha = pt_alpha)) +
  geom_point() +
  scale_color_gradient2(low = "#1696d2", mid = "#cccccc", high = "#db2b27",
  ...
  theme(
    ...
    axis.text        = element_text(size = 8, color = "#aaaaaa"),
  ...
  labs(x = "Poverty Rate", y = "Unemployment Rate")
...
```{r}
#| label: map-state-lisa
#| title: "Spatial Clustering: Economic Hardship Index (LISA 2023)"
maplibre(
  style  = carto_style("dark-matter"),
  bounds = map_bounds
) |>
  ...
  add_line_layer(
    id = "target-border-lisa", source = target_boundary,
    line_color = "#fdbf11", line_width = 2.5
  ) |>
  add_line_layer(
    id = "lisa-borders", source = az_lisa_map,
    line_color = "#2a2a2a", line_width = 0.4, line_opacity = 0.6
  ) |>
  htmlwidgets::onRender("
    function(el, x) {
      var entries = [
        { color: '#db2b27', label: 'Hot Spot (HH) \u2014 High hardship cluster' },
        { color: '#1696d2', label: 'Cold Spot (LL) \u2014 Low hardship cluster' },
        { color: '#fdbf11', label: 'High-Low Outlier (HL)' },
        { color: '#73bfe2', label: 'Low-High Outlier (LH)' },
        { color: '#d2d2d2', label: 'Not Significant (NS)' }
      ];
      var leg = document.createElement('div');
      leg.style.cssText = 'position:absolute;bottom:32px;left:12px;background:rgba(15,15,15,0.82);color:#e8e8e8;font-size:11px;font-family:Lato,sans-serif;padding:10px 13px;border-radius:5px;line-height:1.7;min-width:230px;';
      ...
      title.textContent = 'Economic Hardship Clusters (p < 0.05)';
      leg.appendChild(title);
      entries.forEach(function(e) {
        var row = document.createElement('div');
        row.style.cssText = 'display:flex;align-items:center;margin-bottom:3px;';
        var sw = document.createElement('span');
        sw.style.cssText = 'display:inline-block;width:13px;height:13px;border-radius:2px;margin-right:8px;flex-shrink:0;background:' + e.color + ';';
        var lb = document.createElement('span'); lb.textContent = e.label;
      ...
      badge.innerHTML = '\u25a0&nbsp;Target county boundary';
      el.appendChild(leg); el.appendChild(badge);
    }
  ")
```
...
#| title: "Within-County Variation in Economic Hardship — State Counties"
az_county_hetero_tbl %>%
  DT::datatable(
    options = list(
      pageLength = 15, dom = "t",
      scrollY = "230px", scrollX = TRUE,
      columnDefs = list(list(className = "dt-center", targets = 1:5))
    ),
    rownames = FALSE,
    caption  = htmltools::tags$caption(
      style = "font-size:0.71rem;color:#767676;text-align:left;caption-side:bottom;padding-top:4px;",
      "Sorted best-off to worst-off by mean Economic Hardship Index across county tracts."
    )
  ) %>%
  DT::formatStyle(
    "County", target = "row",
...
### Cluster Stats {width=33%}

```{r}
#| label: vb-state-hh
#| content: valuebox
list(
  title = paste0("Hot Spot Tracts — High Economic Hardship Clusters (", TARGET_STATE, ")"),
  value = paste0(az_n_hh, " Tracts"),
  icon  = "fire", color = "danger"
)
```
...
#| content: valuebox
list(
  title = paste0("Cold Spot Tracts — Low Economic Hardship Clusters (", TARGET_STATE, ")"),
  value = paste0(az_n_ll, " Tracts"),
  icon  = "snow", color = "primary"
)
```

```{r}
#| label: vb-state-moran
#| content: valuebox
list(
  title = paste0("Global Moran's I — Spatial Clustering Strength (", TARGET_STATE, ")"),
  value = az_moran_i,
  icon  = "diagram-3", color = "secondary"
)
```

# County Clusters & Trajectories

## Column {width=50%}

```{r}
#| label: sankey-ehi
#| title: "Economic Hardship Quintile Mobility: Target County (2013–2023)"
ehi_flow <- change_df %>%
  st_drop_geometry() %>%
  dplyr::select(GEOID, hardship_2013) %>%
  left_join(
    local_2023 %>% st_drop_geometry() %>% dplyr::select(GEOID, hardship_2023 = hardship_index),
    by = "GEOID"
  ) %>%
  filter(!is.na(hardship_2013), !is.na(hardship_2023)) %>%
  mutate(q_2013 = ntile(hardship_2013, 5), q_2023 = ntile(hardship_2023, 5)) %>%
  group_by(q_2013, q_2023) %>%
  summarise(n_tracts = n(), .groups = "drop")

q_labels <- c("Q1 Least Hardship", "Q2", "Q3 Middle", "Q4", "Q5 Most Hardship")
q_colors <- colorRampPalette(c("#1696d2", "#d2d2d2", "#db2b27"))(5)

plot_ly(
  type = "sankey", orientation = "h", arrangement = "freeform",
  node = list(
    label     = c(paste0("2013  ", q_labels), paste0("2023  ", q_labels)),
    color     = c(q_colors, q_colors),
    pad = 24, thickness = 28,
    line = list(color = "#cccccc", width = 0.4)
  ),
  link = list(
    source = ehi_flow$q_2013 - 1L,
    target = ehi_flow$q_2023 - 1L + 5L,
    value  = ehi_flow$n_tracts,
    color  = sapply(ehi_flow$q_2013, function(q) {
      h <- q_colors[q]
      sprintf("rgba(%d,%d,%d,0.38)",
              strtoi(substr(h,2,3),16), strtoi(substr(h,4,5),16), strtoi(substr(h,6,7),16))
    }),
    label = paste0(ehi_flow$n_tracts, " tracts | 2013 Q", ehi_flow$q_2013,
                   " \u2192 2023 Q", ehi_flow$q_2023)
  )
) %>%
  layout(
    title  = list(
      text = paste0("EH Quintile Mobility: ", TARGET_LABEL, " Tracts (2013\u20192023)",
                    "<br><sup>Q1 = least hardship &nbsp;\u2502&nbsp; Q5 = most &nbsp;",
                    "\u2502&nbsp; Width = tract count</sup>"),
      font = list(size = 12, color = "#333333")
    ),
    font = list(size = 11, color = "#333333"),
    margin = list(t = 65, b = 20, l = 20, r = 20)
  ) %>%
  config(displayModeBar = FALSE)
```
...
#| title: "Hardship Cluster Trajectories: Target County (2013–2019)"
traj_map <- change_df %>%
  mutate(
...
county_bbox  <- st_bbox(traj_map)
county_bounds <- c(county_bbox$xmin - 0.1, county_bbox$ymin - 0.1,
                   county_bbox$xmax + 0.1, county_bbox$ymax + 0.1)
...
maplibre(
  ...
  htmlwidgets::onRender("
    function(el, x) {
      ...
      var leg = document.createElement('div');
      leg.style.cssText = 'position:absolute;top:12px;left:12px;background:rgba(15,15,15,0.82);color:#e8e8e8;font-size:11px;font-family:Lato,sans-serif;padding:10px 13px;border-radius:5px;line-height:1.7;min-width:220px;';
      var title = document.createElement('div');
      title.style.cssText = 'font-weight:700;font-size:11.5px;margin-bottom:7px;color:#ffffff;border-bottom:1px solid #444;padding-bottom:5px;';
...
#| label: vb-persistent
#| content: valuebox
list(title = "Persistently High Hardship (2013\u20132019)",
...
#| label: vb-emerging
#| content: valuebox
list(title = "Emerging Hot Spots (2013\u20132019)",
...
**`r n_persist_hh` Persistent Hot Spot Tracts (`r pct_persist_hh_23`% of `r TARGET_LABEL` tracts)**

These census tracts had statistically significant high-hardship clustering in **both** 2013 and 2019. The pattern is not random — a Global Moran's I of `r round(moran_global$estimate[1], 3)` confirms that hardship is spatially concentrated. Tracts in this category share infrastructure deficits, limited employment access, and concentrated poverty that reinforce one another across neighborhood boundaries.
...
**`r n_emerg_hh` Emerging Hot Spot Tracts (`r pct_emerg_hh_23`% of `r TARGET_LABEL` tracts)**

These tracts were **not** significant hardship clusters in 2013 but became statistically significant by 2019 — representing the spatial expansion of hardship beyond historically distressed cores.
...
*[TODO: Identify where emerging hot spots are forming and what may be driving displacement.]*
:::

::: {.card style="border-left: 5px solid #55b748;"}
### 🟢 Signs of Progress — Read With Caution

**`r pct_improved_23`% of tracts improved (2013→2023); `r pct_worsened_23`% worsened**

The majority of tracts improved over this window. However, the data cannot distinguish genuine economic uplift from population turnover — a tract with a declining hardship index may simply have replaced lower-income residents with higher-income newcomers.
...
::: {.card}
### Recommendation 1
**Target:** `r n_persist_hh` Persistent Hot Spot tracts — spatially concentrated, entrenched hardship confirmed by Global Moran's I = `r round(moran_global$estimate[1], 3)`.

**TODO:** Name the specific geographic corridor, cite hardship index values, and propose a concrete place-based intervention with a named responsible entity.
...
::: {.card}
### Recommendation 2
**Target:** `r n_emerg_hh` Emerging Hot Spot tracts — new high-hardship clusters not present in 2013.

**TODO:** Reference the displacement paradox, identify where clusters are forming, and propose an early-intervention or monitoring strategy.
...
::: {.card}
### Recommendation 3
**Evidence base:** `r pct_improved_23`% improved, `r pct_worsened_23`% worsened (2013→2023); Moran's I = `r round(moran_global$estimate[1], 3)`.

**TODO:** Using the trajectory map and Sankey, make a forward-looking data-monitoring or cross-sector coordination argument.
