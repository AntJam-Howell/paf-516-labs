...
subtitle: "Instructor Example | Completed Analysis"
format:
  dashboard:
    theme:
      - flatly
...
library(viridis); library(patchwork); library(biscale); library(tigris); library(DT); library(mapgl); library(glue); library(plotly)
library(scales)
```
...
az_counties <- county_2023 %>%
  ...
  mutate(
    ...
    `AZ Rank` = case_when(
      az_rank == 1   ~ "1 - Worse Off",
      az_rank == n_az ~ paste0(n_az, " - Better Off"),
      TRUE           ~ as.character(az_rank)
    )
  ) %>%
  select(`AZ Rank`, County, `Economic Hardship Index`,
         `US Rank` = us_rank)
```
...
# Arizona state boundary (clean outline, no internal county lines)
...
maplibre(
  ...
  add_legend(
    ...
    values  = c("Worse-off", "", "Better-off"),
    colors  = c("#db2b27", "#f5f5f5", "#1696d2"),
...
p <- ggplot(compare_df,
  ...
  theme(
    ...
    axis.text.x        = element_text(size = 10, color = "#767676"),
...
# Midpoint label for hover on the AZ mean line
mid_label <- levels(compare_df$label)[ceiling(nlevels(compare_df$label) / 2)]

ggplotly(p, tooltip = "text") %>%
  add_markers(
    x = az_mean, y = mid_label,
    marker = list(color = "#9d9d9d", size = 8, symbol = "line-ns-open",
                  line = list(width = 0)),
    hoverinfo = "text",
    text = paste0("AZ State Average\nIndex: ", round(az_mean, 3)),
    showlegend = FALSE, inherit = FALSE
  ) %>%
  layout(
    title = list(
      text = paste0("All Arizona Counties vs. U.S. Extremes",
                    "<br><sup style='color:#767676'>",
                    "<span style=\"color:#fdbf11\">\u25CF</span> AZ Counties &nbsp; ",
