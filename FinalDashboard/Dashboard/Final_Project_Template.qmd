...
::: {.card}
**Economic Hardship Clusters in `r TARGET_LABEL` (LISA)**

County-level maps mask enormous within-county variation. Apache and Navajo counties show uniformly high hardship across nearly all census tracts — the county average *is* the tract-level story. A large, diverse county like Maricopa is different: its county-level score averages over tracts ranging from deep poverty to affluent suburbs, making hardship hot spots invisible at the county scale. Local Indicators of Spatial Association (LISA) solve this by testing each tract against its neighbors, identifying statistically significant clusters of high hardship (hot spots) and low hardship (cold spots) that county averages obscure.
:::
...
```{r}
#| label: vb-lisa-summary
#| output: asis
cat(glue::glue(
  '<div style="display:flex;gap:12px;justify-content:center;padding:4px 8px;">
  <span style="background:#db2b27;color:#fff;padding:4px 12px;border-radius:4px;font-size:0.85rem;font-weight:600;">🔴 {n_hh} Hot Spot Tracts</span>
  <span style="background:#1696d2;color:#fff;padding:4px 12px;border-radius:4px;font-size:0.85rem;font-weight:600;">🔵 {n_ll} Cold Spot Tracts</span>
  <span style="background:#6c757d;color:#fff;padding:4px 12px;border-radius:4px;font-size:0.85rem;font-weight:600;">Moran\'s I = {round(unname(moran_global$estimate[1]), 3)}</span>
  </div>'
))
```
