...
| **Local Patterns** | Block group hardship map linked to a detail table via crosstalk | Leaflet map + DT table (linked) |
...
- **At least 1 crosstalk-linked leaflet map + DT table**: The Local Patterns tab must use `crosstalk::SharedData` to link a `leaflet` map to a `DT::datatable`. Selecting rows in the table should highlight polygons on the map, and clicking polygons on the map should select table rows.
...
- **Self-contained HTML under 25 MB**. Use static `ggplot` maps (not leaflet) for the national choropleth and LISA map to keep file size manageable. Reserve leaflet for the one interactive crosstalk panel.
...
- **Crosstalk interactivity (15 pts)**: Clicking a polygon on the leaflet map selects the corresponding row in the DT table, and selecting a row highlights the polygon. Both directions must work.
...
- **File too large**: If your HTML exceeds 25 MB, switch the national map from leaflet to a static ggplot. Use `geom_sf()` with `color = NA` to avoid rendering individual county borders (which inflates file size).
- **Crosstalk not working on RPubs**: Ensure you are using `crosstalk::SharedData$new()` with a `key` argument (e.g., `key = ~GEOID`). Both the leaflet map and the DT table must reference the **same** SharedData object.
...
- **Maps not displaying**: Confirm that `sf` geometries are transformed to WGS84 (`st_transform(data, 4326)`) before passing to leaflet.
